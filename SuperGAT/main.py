...
def run(args, gpu_id=None, return_model=False, return_time_series=False):
    ...
    net = net_cls(args, train_d)  # 初始化supergat
    ...
    for current_iter, epoch in enumerate(tqdm(range(args.start_epoch, args.start_epoch + args.epochs))):
        # train_model可以得到节点表示
        train_loss,node2vec = train_model(running_device, net, train_d, loss_func, adam_optim, epoch=epoch, _args=args)
...
def run_with_many_seeds(args, num_seeds, gpu_id=None, **kwargs):
    ...
    for i in range(num_seeds):
        ...
        ret = run(_args, gpu_id=gpu_id, **kwargs)  # 模型运行？
...
if __name__ == '__main__':
    ...
    main_args.num_hidden_features = 128
    ...
    many_seeds_result,vector = run_with_many_seeds(main_args, num_total_runs, gpu_id=alloc_gpu[0])

    print(vector.shape)
    doc_vectors.extend(vector.data.tolist())
    np.save('./pretrain/EV13NSO8_citeseer_doc_vectors_128', doc_vectors)
