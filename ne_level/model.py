...
class SuperGATNet(nn.Module):
    ...
    def __init__(self, args, dataset_or_loader):
        ...
        gat_cls = _get_gat_cls(self.args.model_name)  # 获得模型名称

        num_input_features = getattr_d(dataset_or_loader, "num_node_features")
        # num_classes = getattr_d(dataset_or_loader, "num_classes")
        num_classes = 128  # the same with the main_args.num_hidden_features in main.py
        ...
        self.conv2 = gat_cls(
            ...
            args.num_hidden_features * args.heads, num_classes,  # 这里的值应该是和加载预向量对应的,后者是16
    ...
    def forward(self, x, edge_index, batch=None, **kwargs) -> torch.Tensor:
        # print("sg导入的x是否为gpu数据：",x.device)
        x = F.dropout(x, p=self.args.dropout, training=self.training)  # (2708,1433)
        x = self.conv1(x, edge_index, batch=batch, **kwargs)  # (2708,512)
        x = F.elu(x)

        x = F.dropout(x, p=self.args.dropout, training=self.training)
        x = self.conv2(x, edge_index, batch=batch, **kwargs)  # 节点表示，(2708,128)

        if self.training and self.args.verbose >= 3:
            _inspect_attention_tensor(x, edge_index, self.conv2.cache)

        # A_pred = self.dot_product_decode(x)  # no-ori
